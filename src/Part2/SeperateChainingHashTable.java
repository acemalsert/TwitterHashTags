...
public class SeperateChainingHashTable<Key, Value> {
    ...
    public Iterable MostPopularHashTags(){
        Part1.Queue<Key> queue = new Part1.Queue<Key>();
        for (int i = 0; i < M; i++) {
            for (Object key : st[i].popularKeysAndCounts())
                if(queue.size() < 10) queue.enqueue((Key) key);
                else break;
        }
        return queue;
    }
    ...
    public static void main(String [] args){
        ...
        //Reader.readHashtags1(fileName,seperateChainingHashTable);
        Reader.readHashtags1(anotherFile,seperateChainingHashTable);
